\section{Estimation}
 ...
 \frame{\sectionpage}
    ...
    \begin{frame}{Estimation of Weighted Treatment Effects}
        ...
        \uncover<1->{
            What's the weighted treatment effects to conduct inference for?
            \uncover<2->{
                $$
    ...
    \begin{frame}{Weighted Treatment Effects: Example}
        $$
        ...
        \uncover<1->{
            \begin{itemize}
                \item RD paramater:
                \begin{align*}
                    \tau_{c}=\mathbb{E}\left[ Y_{i}\left(1\right)-Y_{i}\left(0\right)\mid Z_{i}=c\right] &= \mathbb{E}\left[\tau(U_i)\mid Z_i=c\right]\\
                    &= \int\frac{p\left(c\mid u\right)}{ \only<1>{\int p\left(c\mid u\right)\mathrm{d}G\left(u\right)} \only<2>{\textcolor{mygreen}{\underbrace{\int p\left(c\mid u\right)\mathrm{d}G\left(u\right)}_{=f_G(c)} }} } \tau\left(u\right) \mathrm{d}G\left(u\right)
                \end{align*}
            \end{itemize}
        }
    ...
    %\begin{frame}{Weighted Treatment Effects: Example 2}
    %    $$
    %    \tau_{w}=\int\frac{w\left(u\right)}{\mathbb{E}_{G}\left[w\left(U\right)\right]} {\tau\left(u\right)} \mathrm{d}G\left(u\right),w\left(\cdot\right)\geq 0
    %    $$
    ...
    \begin{frame}{Asymptotic Bias}
        \begin{block}{\textbf{Theorem: Asymptotic Limit of $\hat{\tau}_\gamma$}}
            ...
            \end{align*}
        \end{block}
        \uncover<3->{where $$h\left(u,\gamma\right)\coloneqq\int\gamma\left(z\right)p\left(z\mid u\right)\mathrm{d}\lambda\left(z\right),\ \alpha_{\left(w\right)}\left(u\right)=\mathbb{E}\left[Y_{i}\left(w\right)\mid U_{i}=u\right]$$}
    ...
    \begin{frame}{Asymptotic Bias: Decomposition}
        ...
        \uncover<2->{\begin{itemize}
            ...
            \uncover<5->{
                \begin{itemize}
                    \uncover<5->{\item[-] $\tau(u)$ being constant w.r.t. $u$, a constant conditional treatment effect}
                    \uncover<6->{\item[-] $h(u,\gamma_+)=w(u),\forall u$, an absolutely ``correct" weighting function}
                \end{itemize}
            }
        \end{itemize}}
        \vspace*{15pt}
