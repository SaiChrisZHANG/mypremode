    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.45\textwidth}
            \begin{itemize}
                ...
                \item[A2] \textcolor{mygreen}{\underline{\textbf{Noisy}}} $Z_i$: $Z_i\mid U_i \sim {p\left(\cdot \mid U_i\right)}$
...
\begin{frame}{Sharp RD Design with A Noisy Running Variable}
    ...
    %\uncover<6->{
    %    \vspace*{5pt}
    %    To achieve balance in the \textcolor{mygreen}{\textbf{\underline{latent variable}}}: $h(\cdot,\gamma_+)\approx h(\cdot,\gamma_-)$
    %    \vspace*{10pt}
    %}
...
\begin{frame}{Sharp RD Design with A Noisy Running Variable}
    ...
    \uncover<2->{
        \vspace*{15pt}
        \begin{itemize}
            \item \textcolor{mygreen}{\textbf{\underline{No need}}} to know $G$ {\footnotesize (distribution of $U$)}
            \item \textcolor{mygreen}{\textbf{\underline{Need}}} to know \textcolor<3->{mygreen}{$p(z\mid u)$} {\footnotesize (conditional distribution of the noise)}
            \begin{itemize}
                \item<3->[-] test-retest data, prior modelling of responses to tests, physical model of the measurement device, biomedical knowledge, etc.
                \item<4->[-] still valid when \textcolor{mygreen}{\textbf{underestimating}} the true noise level
            \end{itemize}
