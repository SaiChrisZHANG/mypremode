...
\begin{frame}{RD Identification: Measurement Error}
    ...
    \begin{center}
       ...
       \uncover<1->{$\textcolor<2->{mygreen}{\underbrace{U_i}_{\text{{latent variable}}}}$} & \uncover<3->{$\xRightarrow{Z_i=U_i+\textcolor<4->{mygreen}{e_i}}$} & $\underbrace{Z_i}_\text{running variable}$ & $\xRightarrow{W_i=\mathbf{1}\left(\{Z_i\geq {\textcolor{mygreen}{c}}\}\right)}$ & $\underbrace{W_i}_\text{treatment}$ & $\Rightarrow$ & $\underbrace{Y_i}_{\text{outcome}}$\\
        &\\
           \hline
           & \\
